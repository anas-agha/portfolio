<template lang="">
  <!-- desktop nav -->
  <header ref="navDesktop" class="nav-bar desktop">
    <div class="container">
      <div style="direction: var(--dir)" class="routes">
        <div v-for="route in routes.filter((rt) => !rt.dontShow)" :key="route.path" class="route">
          <router-link :to="{ path: route.path }">{{
            lang === 'en' ? route.name : route.nameAr
          }}</router-link>
        </div>
      </div>
      <div class="togglers">
        <div @click="$i18n.locale = changeLang($i18n.locale)" style="perspective: 50px">
          <div
            :style="lang == `en` ? 'transform: rotateX(180deg) ' : 'transform: rotateX(0deg)'"
            class="languge-toggler"
          >
            <div v-html="en" class="en"></div>
            <div v-html="ar" class="ar"></div>
          </div>
        </div>
        <div @click="switchTheme" class="theme-toggler">
          <!-- <div class="toggling-container"></div> -->
          <div
            :style="theme == 'light' ? 'transform: translate(40px);' : 'transform: translate(0);'"
            class="ball"
          ></div>
  ...
  <!-- navbar mobile -->
  <header ref="navMobile" class="nav-bar mobile">
    <div class="container">
      <div class="togglers">
        <div @click="$i18n.locale = changeLang($i18n.locale)" style="perspective: 50px">
          <div
        ...
        <div @click="mobileMenu = !mobileMenu" class="menu-icon">
          <span></span>
          <span></span>
          <span></span>
        </div>
    ...
    <div :class="`menu ${mobileMenu ? 'active' : ''}`">
      <div class="container">
        <p
          @click="mobileMenu = !mobileMenu"
          class="close"
          :style="`--i:${routeDelay(routesLength - 1)}`"
        >
          <font-awesome-icon :icon="['far', 'circle-xmark']" />
        </p>
        <div style="direction: var(--dir)" class="routes">
          <div
            v-for="(route, index) in routes.filter((rt) => !rt.dontShow)"
            :key="route.path"
            class="route"
            @click="mobileMenu = false"
          >
            <router-link :style="`--i:${routeDelay(index)}`" :to="{ path: route.path }">{{
              lang === 'en' ? route.name : route.nameAr
            }}</router-link>
          </div>
        </div>
...
const switchTheme = store.switchTheme
const lang = storeToRefs(store).lang
const changeLang = store.changeLang
...
const routesLength = computed(() => routes.length + 2)
watch(mobileMenu, bodyScroll)
function routeDelay(index) {
  if (mobileMenu.value) {
    return index + 1
  } else {
    return routes.length - index
  }
}
watch(mobileMenu, routeDelay)
...
onMounted(() => {
  ...
  function showAndHideNav() {
    ...
    if (scrollY.value < window.scrollY && window.scrollY > 60 && scrolling) {
      navDesktop.value.style.transform = 'translateY(-62px)'
      if (!mobileMenu.value) {
        navMobile.value.style.transform = 'translateY(-62px)'
      }
    } else {
      navMobile.value.style.transform = 'translateY(0)'
      navDesktop.value.style.transform = 'translateY(0)'
    }
...
.nav-bar {
  ...
  box-shadow: 0 0 6px 0px var(--active-color);
  display: flex;
  ...
  .container {
    ...
    .togglers {
      ...
      .theme-toggler {
        ...
        padding: 3px;
        background-color: var(--back-color);
        box-shadow: 0 0 5px 1px var(--active-color);
...
@media (max-width: 575px) {
  ...
  .nav-bar.mobile {
    ...
    .menu {
      ...
      transition: transform 0.5s calc(v-bind(routesLength) * 0.3s);
          ...
          .route {
            ...
            a {
              ...
              transition:
                ...
                font-weight 0.3s,
                transform 0.3s calc(var(--i) * 0.3s),
                opacity 0.3s calc(var(--i) * 0.3s);
              &.router-link-active,
              &:hover {
                background-color: var(--active-color);
  ...
  .nav-bar.mobile .menu.active .container p {
    scale: 1;
